...
class RpcServer {
  ...
  async autoConfig (address, server) {
    ...
    for (let i = 0; i < len; i++) {
      ...
      server.addProtoService(proto[p.name].service, await this.generateImplementation([p.name], [pointFunction]));
    }
  }
  ...
  async manualConfig (address, server) {
    ...
    for (let i = 0; i < len; i++) {
      ...
      server.addProtoService(proto[serviceName].service, await this.generateImplementation(p.functionArray, p.pointFunArray));
    }
  }

  async generateImplementation (functionArray, pointFunArray) {
